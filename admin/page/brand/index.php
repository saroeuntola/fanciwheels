...
<!-- page content  -->
        ...
        <h2 class="text-2xl font-bold text-center mb-6">Brand List</h2>

        <!-- Create Product Button -->
        <div class="flex justify-end mb-4">
            <a href="create.php" class="bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700">+ Add Brand</a>
        </div>
            ...
            <table class="min-w-full bg-white border border-gray-200 rounded-lg shadow-md">
                <thead>
                    <tr class="text-left text-sm font-medium text-gray-600 bg-gray-50">
                        ...
                        <th class="px-4 py-3 border-b">Brand Name</th>
    ...
    <?php foreach ($brands as $row): ?>
        <tr class="text-gray-700 hover:bg-gray-50 transition">
            <td class="px-4 py-3 border-b">
                <img src="<?= htmlspecialchars($row['brand_image']) ?>" alt="Brand" class="h-12 w-12 object-cover rounded-md">
            </td>
            <td class="px-4 py-3 border-b"><?= htmlspecialchars($row['brand_name']) ?></td>
            <td class="px-4 py-3 border-b"><?= htmlspecialchars($row['link']) ?></td>
            ...
            <td class="px-4 py-3 border-b">
                ...
                <a href="delete.php?id=<?= $row['id'] ?>" class="bg-red-600 text-white px-3 py-1 rounded-md hover:bg-red-700" onclick="return confirm('Are you sure you want to delete this brands?');">Delete</a>
            </td>
        </tr>
    <?php endforeach; ?>
<?php else: ?>
    <tr>
        <td colspan="8" class="px-4 py-3 text-center text-gray-500">No brands found</td>
    </tr>
<?php endif; ?>
